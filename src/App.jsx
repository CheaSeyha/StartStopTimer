...
function App() {
  ...
  useEffect(() => {
    ...
    const savedHistory = JSON.parse(localStorage.getItem('timerHistory') || '[]')
        ...
        <div className="flex-grow flex items-center justify-center p-6 pb-24 overflow-y-auto">
            ...
            {view === 'timer' ? (
                ...
                <div className="relative z-10 w-full max-w-sm backdrop-blur-3xl bg-white/5 border border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] rounded-[3rem] p-10 flex flex-col items-center">
                    <h1 className="text-white/80 text-lg tracking-widest uppercase font-medium mb-12">Timer Tracker</h1>
                    ...
                    <div className="relative group">
                        <div className={`absolute inset-0 bg-gradient-to-r ${isRunning ? 'from-red-500 to-orange-500' : 'from-cyan-400 to-blue-600'} rounded-full blur opacity-40 group-hover:opacity-75 transition duration-500 animate-pulse`}></div>
                        <button 
                            onClick={toggleTimer}
                            className={`
                        ...
                        >
                            {isRunning ? (
                                ...
                                <>
                                    ...
                                    <span className="text-sm font-bold tracking-widest uppercase pl-1">Start</span>
                                </>
                            )}
                        </button>
                    </div>

                    <p className={`mt-12 text-sm font-medium tracking-wide transition-colors duration-500 ${isRunning ? 'text-green-400/80' : 'text-white/30'}`}>
                ...
                <div className="relative z-10 w-full max-w-sm h-[600px] backdrop-blur-3xl bg-white/5 border border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] rounded-[3rem] p-6 flex flex-col">
                    <h1 className="text-white/80 text-lg tracking-widest uppercase font-medium mb-6 text-center">History</h1>
                    ...
                    <div className="flex-grow overflow-y-auto space-y-3 pr-2 scrollbar-hide">
                        {history.length === 0 ? (
                            ...
                            history.map((session) => (
                                <HistoryItem key={session.id} session={session} onDelete={() => {
                                    const newHistory = history.filter(h => h.id !== session.id)
        ...
        <div className="relative h-20 overflow-hidden rounded-xl">
            ...
            >
                <div className="flex justify-between items-center mb-1 pointer-events-none select-none">
                    <span className="text-white/90 font-mono text-lg">{session.durationDisplay}</span>
                    <span className="text-cyan-400 text-sm font-medium">{session.durationHours} hrs</span>
                </div>
